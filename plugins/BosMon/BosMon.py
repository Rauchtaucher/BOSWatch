...
import urllib.parse #for the HTTP request with parameters
		...
		if configHandler.checkConfig("BosMon"): #read and debug the config
			...
			try:
				...
				if globalVars.config.get("BosMon", "bosmon_user"):
					...
					headers['Authorization'] = "Basic {0}".format(base64.b64encode("{0}:{1}".format(globalVars.config.get("BosMon", "bosmon_user"), globalVars.config.get("BosMon", "bosmon_password")).encode()).decode())
